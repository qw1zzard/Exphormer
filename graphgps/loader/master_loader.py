...
from graphgps.transform.transforms import (
    generate_splits,
)  # not the same as split_generator above.
from graphgps.transform.transforms import (
    clip_graphs_to_size,
    concat_x_and_pos,
    move_node_feat_to_x,
...
def load_dataset_master(format, name, dataset_dir):
    ...
    if cfg.prep.dist_enable:
        start = time.perf_counter()
        logging.info('Precalculating node distances and shortest paths ...')
    ...
    if cfg.posenc_ERN.enable or cfg.posenc_ERE.enable:
        start = time.perf_counter()
        logging.info('Precalculating effective resistance for graphs ...')
